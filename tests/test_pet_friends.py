...
"""ниже блок с простеньким тестом для валидного пользователя
...
"""ниже такой же тест, но для невалидного пользователя
...
"""верный пароль, но невалидная почта, чтобы наверняка убедиться, что пропускаются только верные комбинации
как и выше, проверяем, что пользователь получит код из документации"""
...
def test_add_new_pet_with_valid_data(name='Шрек', animal_type='троль', age='44', pet_photo='images/shrek1.jpg'):
...
def test_add_new_pet_with_invalid_data(name='Шрек', animal_type='троль', age='44', pet_photo='images/shrek1.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:   #если длина json соответствует 0
        pf.add_new_pet(auth_key, "Шкек", "тролль", '88', "images/shrek2.jpg") #то добавляем инфу о новом питомце
...
def test_simple_add_new_pet_with_valid_data(name='Муся', animal_type='дворовая', age='3'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet_without_photo(auth_key, name, animal_type, age)
    assert status == 200
    assert result['name'] == name

def test_simple_add_new_pet_with_unvalid_data(name='Дик', animal_type='0', age='4'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet_without_photo(auth_key, name, animal_type, age)
    assert status == 400
    assert result['name'] != name
...
def test_add_photo_with_valid_data(pet_photo='images/shrek2.jpg'):
